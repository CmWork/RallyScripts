...
# python EndOfSprintReport.py -p "MBH - HNL" -s "MBH-HNL Sprint 55" -d "true"
...
def main(argv):
    ...
    toWrite = False
    try:
        opts, args = getopt.getopt(argv, "hp:s:d:", ["project=","sprint=","db="])
    except getopt.GetoptError:
        print 'python EndOfSprintReport.py -p <project name> -s <sprint name> -d <true|false>'
        sys.exit(2)
    for opt, arg in opts:
        if opt == "-h":
            print 'python EndOfSprintReport.py -p <project name> -s <sprint name> -d <true|false>'
            ...
            projectName = arg
        elif opt in ("-s", "--sprint"):
            sprintName = arg
        elif opt in ("-d", "--db"):
            if arg == 'true':
                toWrite = True

    if projectName is not None and sprintName is not None:
        ...
        print "Project name or Sprint name not set. Both need to exist."
